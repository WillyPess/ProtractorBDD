...
export class ExpectedHelper {
    ...
    waitForElementContain(element, text) {
        return browser.wait(browser.ExpectedConditions.textToBePresentInElement(element, text), 1000 * 4)
    ...
    waitForElementIsPresent(element) {
        return browser.wait(browser.isElementPresent(element), 1000 * 10)
